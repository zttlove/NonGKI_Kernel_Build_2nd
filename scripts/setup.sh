...
@@ -1592,6 +1610,14 @@ static struct dentry *lookup_dcache(const struct qstr *name,
...
@@ -1609,14 +1635,27 @@ static struct dentry *__lookup_hash(const struct qstr *name,
 	struct dentry *old;
...
@@ -1625,6 +1664,26 @@ static struct dentry *__lookup_hash(const struct qstr *name,
 		dput(dentry);
 		dentry = old;
 	}
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -1635,6 +1694,9 @@ static int lookup_fast(struct nameidata *nd,
...
+#endif
 	...
 	/*
@@ -1646,6 +1708,16 @@ static int lookup_fast(struct nameidata *nd,
 		unsigned seq;
 		bool negative;
...
+#endif
 		if (unlikely(!dentry)) {
 			if (unlazy_walk(nd))
 				return -ECHILD;
@@ -1692,6 +1764,16 @@ static int lookup_fast(struct nameidata *nd,
...
+#endif
 		if (unlikely(!dentry))
 			return 0;
 		status = d_revalidate(dentry, nd->flags);
@@ -1723,12 +1805,19 @@ static struct dentry *__lookup_slow(const struct qstr *name,
...
@@ -1882,6 +1990,11 @@ static int walk_component(struct nameidata *nd, int flags)
 	if (unlikely(err <= 0)) {
 		if (err < 0)
 			return err;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -2160,6 +2273,17 @@ static int link_path_walk(const char *name, struct nameidata *nd)
 		if (err)
 			return err;
...
+
 		...
 		type = LAST_NORM;
@@ -2343,6 +2467,9 @@ static inline int lookup_last(struct nameidata *nd)
 {
...
@@ -3282,15 +3409,39 @@ static int lookup_open(struct nameidata *nd, struct path *path,
...
@@ -3414,6 +3565,10 @@ static int do_last(struct nameidata *nd,
 	struct path path;
 	int error;
...
@@ -3623,9 +3778,30 @@ static int do_tmpfile(struct nameidata *nd, unsigned flags,
...
+#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
 		...
 		path_put(&path);
@@ -3666,6 +3863,11 @@ static int do_o_path(struct nameidata *nd, unsigned flags, struct file *file)
...
@@ -3684,8 +3886,33 @@ static struct file *path_openat(struct nameidata *nd,
...
+#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
 	if (likely(!error)) {
 		if (likely(file->f_mode & FMODE_OPENED))
 			return file;
@@ -3702,6 +3929,10 @@ static struct file *path_openat(struct nameidata *nd,
 	return ERR_PTR(error);
 }
...
+
 struct file *do_filp_open(int dfd, struct filename *pathname,
 		const struct open_flags *op)
 {
@@ -4895,7 +5126,18 @@ int vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen)
...
+#endif
 		...
 		if (!d_is_symlink(dentry))
 			return -EINVAL;
@@ -4911,6 +5153,15 @@ int vfs_readlink(struct dentry *dentry, char __user *buffer, int buflen)
 		if (IS_ERR(link))
 			return PTR_ERR(link);
 	}
+#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
...
@@ -114,6 +125,12 @@ static int mnt_alloc_id(struct mount *mnt)
...
@@ -122,7 +139,24 @@ static void mnt_free_id(struct mount *mnt)
  */
...
+#endif
 	...
 	if (res < 0)
 		return res;
@@ -172,6 +206,116 @@ int mnt_get_count(struct mount *mnt)
 #endif
 }
...
+
 ...
 {
 	struct mount *mnt = kmem_cache_zalloc(mnt_cache, GFP_KERNEL);
@@ -612,6 +756,18 @@ struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry)
...
+
 	hlist_for_each_entry_rcu(p, head, mnt_hash)
 		if (&p->mnt_parent->mnt == mnt && p->mnt_mountpoint == dentry)
 			return p;
@@ -941,7 +1097,21 @@ struct vfsmount *vfs_create_mount(struct fs_context *fc)
...
@@ -1034,7 +1204,40 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
 	struct mount *mnt;
 	int err;
...
@@ -1060,6 +1263,13 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
...
@@ -3295,6 +3505,9 @@ struct mnt_namespace *copy_mnt_ns(unsigned long flags, struct mnt_namespace *ns,
...
+#endif
 	...
 	if (IS_ERR(new)) {
 		namespace_unlock();
@@ -4074,3 +4287,43 @@ const struct proc_ns_operations mntns_operations = {
...
@@ -73,7 +92,11 @@ static void show_mark_fhandle(struct seq_file *m, struct inode *inode)
...
+#endif
 {
 	...
 	struct inode *inode;
@@ -84,6 +107,60 @@ static void inotify_fdinfo(struct seq_file *m, struct fsnotify_mark *mark)
...
@@ -1795,6 +1802,17 @@ static int do_proc_readlink(struct path *path, char __user *buffer, int buflen)
 	if (!tmp)
 		return -ENOMEM;
...
+
 	...
 	if (IS_ERR(pathname))
@@ -2364,6 +2382,10 @@ proc_map_files_readdir(struct file *file, struct dir_context *ctx)
...
@@ -53,10 +64,69 @@ static int seq_show(struct seq_file *m, void *v)
 	if (ret)
 		return ret;
...
+#endif // #if defined(CONFIG_KSU_SUSFS_SUS_KSTAT) || defined(CONFIG_KSU_SUSFS_SUS_MAP) || defined(CONFIG_KSU_SUSFS_OPEN_REDIRECT)
 ...
 #include <asm/elf.h>
 #include <asm/tlb.h>
@@ -349,6 +352,14 @@ static void show_vma_header_prefix(struct seq_file *m,
 	seq_putc(m, ' ');
 }
...
+
 ...
 {
@@ -363,9 +374,36 @@ show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
...
+#endif // #ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
 	...
 	start = vma->vm_start;
@@ -892,6 +930,13 @@ static void show_smap_vma(struct seq_file *m, void *v)
...
@@ -960,7 +1005,16 @@ static int show_smaps_rollup(struct seq_file *m, void *v)
...
@@ -1626,6 +1680,9 @@ static ssize_t pagemap_read(struct file *file, char __user *buf,
...
+#endif
 	...
 	if (!mm || !mmget_not_zero(mm))
 		goto out;
@@ -1680,7 +1737,15 @@ static ssize_t pagemap_read(struct file *file, char __user *buf,
...
+
 ...
 {
@@ -306,16 +470,34 @@ static int mounts_release(struct inode *inode, struct file *file)
...
@@ -137,6 +142,9 @@ struct old_linux_dirent {
...
@@ -147,6 +155,9 @@ static int fillonedir(struct dir_context *ctx, const char *name, int namlen,
...
+#endif
 	...
 	if (buf->result)
 		return -EINVAL;
@@ -158,6 +169,18 @@ static int fillonedir(struct dir_context *ctx, const char *name, int namlen,
...
+#endif
 	...
 	dirent = buf->dirent;
 	if (!access_ok(dirent,
@@ -185,15 +208,35 @@ SYSCALL_DEFINE3(old_readdir, unsigned int, fd,
...
@@ -213,6 +256,9 @@ struct linux_dirent {
...
+#endif
 	...
 	int count;
 	int error;
@@ -228,6 +274,9 @@ static int filldir(struct dir_context *ctx, const char *name, int namlen,
...
@@ -243,6 +292,18 @@ static int filldir(struct dir_context *ctx, const char *name, int namlen,
...
@@ -277,6 +338,10 @@ SYSCALL_DEFINE3(getdents, unsigned int, fd,
 		.current_dir = dirent
 	};
 	int error;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	if (!access_ok(dirent, count))
 		return -EFAULT;
@@ -285,6 +350,18 @@ SYSCALL_DEFINE3(getdents, unsigned int, fd,
 	if (!f.file)
 		return -EBADF;
...
+#endif
 	...
 	if (error >= 0)
 		error = buf.error;
@@ -298,12 +375,19 @@ SYSCALL_DEFINE3(getdents, unsigned int, fd,
...
+#endif
 	...
 	int count;
 	int error;
@@ -318,6 +402,9 @@ static int filldir64(struct dir_context *ctx, const char *name, int namlen,
...
+#endif
 	...
 	buf->error = verify_dirent_name(name, namlen);
 	if (unlikely(buf->error))
@@ -328,7 +415,20 @@ static int filldir64(struct dir_context *ctx, const char *name, int namlen,
 	prev_reclen = buf->prev_reclen;
 	if (prev_reclen && signal_pending(current))
 		return -EINTR;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+
 	...
 	if (!user_access_begin(prev, reclen + prev_reclen))
 		goto efault;
@@ -363,6 +463,10 @@ int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent,
 		.current_dir = dirent
 	};
 	int error;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	if (!access_ok(dirent, count))
 		return -EFAULT;
@@ -371,6 +475,18 @@ int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent,
 	if (!f.file)
 		return -EBADF;
...
+#endif
 	...
 	if (error >= 0)
 		error = buf.error;
@@ -385,6 +501,10 @@ int ksys_getdents64(unsigned int fd, struct linux_dirent64 __user *dirent,
 			error = count - buf.count;
 	}
 	fdput_pos(f);
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -406,6 +526,9 @@ struct compat_old_linux_dirent {
...
@@ -417,6 +540,9 @@ static int compat_fillonedir(struct dir_context *ctx, const char *name,
...
+#endif
 	...
 	if (buf->result)
 		return -EINVAL;
@@ -428,6 +554,18 @@ static int compat_fillonedir(struct dir_context *ctx, const char *name,
 		buf->result = -EOVERFLOW;
 		return -EOVERFLOW;
 	}
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	dirent = buf->dirent;
 	if (!access_ok(dirent,
@@ -455,15 +593,35 @@ COMPAT_SYSCALL_DEFINE3(old_readdir, unsigned int, fd,
...
@@ -477,6 +635,9 @@ struct compat_linux_dirent {
...
+#endif
 	...
 	int count;
 	int error;
@@ -491,6 +652,9 @@ static int compat_filldir(struct dir_context *ctx, const char *name, int namlen,
 	compat_ulong_t d_ino;
...
+#endif
 	...
 	if (reclen > buf->count)
@@ -507,6 +671,18 @@ static int compat_filldir(struct dir_context *ctx, const char *name, int namlen,
...
+#endif
 	...
 	if (__put_user(d_ino, &dirent->d_ino))
 		goto efault;
@@ -539,6 +715,10 @@ COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
 		.count = count
 	};
 	int error;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
+#endif
 	...
 	if (!access_ok(dirent, count))
 		return -EFAULT;
@@ -547,6 +727,18 @@ COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
 	if (!f.file)
 		return -EBADF;
...
+#endif
 	...
 	if (error >= 0)
 		error = buf.error;
@@ -558,6 +750,10 @@ COMPAT_SYSCALL_DEFINE3(getdents, unsigned int, fd,
 			error = count - buf.count;
 	}
 	fdput_pos(f);
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -77,9 +82,53 @@ int vfs_getattr_nosec(const struct path *path, struct kstat *stat,
...
+#endif // #ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
 	...
 	generic_fillattr(inode, stat);
 	return 0;
@@ -165,6 +214,7 @@ EXPORT_SYMBOL(vfs_statx_fd);
  *
...
@@ -81,9 +85,72 @@ int vfs_get_fsid(struct dentry *dentry, __kernel_fsid_t *fsid)
 }
...
@@ -1085,6 +1093,22 @@ int get_anon_bdev(dev_t *p)
 {
 	int dev;
...
@@ -657,8 +660,36 @@ static int s_show(struct seq_file *m, void *p)
...
index fa3f48a..9c19c2b 100644
--- a/kernel/sys.c
+++ b/kernel/sys.c
@@ -1240,12 +1240,20 @@ static int override_release(char __user *release, size_t len)
 	return ret;
 }
...
@@ -4993,12 +4997,21 @@ int __access_remote_vm(struct task_struct *tsk, struct mm_struct *mm,
...
index 7e1e6bc..7b5c6a6 100644
--- a/security/selinux/avc.c
+++ b/security/selinux/avc.c
@@ -693,6 +693,11 @@ static void avc_audit_pre_callback(struct audit_buffer *ab, void *a)
...
@@ -719,6 +724,19 @@ static void avc_audit_post_callback(struct audit_buffer *ab, void *a)
...
@@ -726,6 +744,9 @@ static void avc_audit_post_callback(struct audit_buffer *ab, void *a)
 		kfree(scontext);
 	}
